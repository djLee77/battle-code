...
export default function Room({ data, dockLayoutRef }: IProps) {
  ...
  const handleRoomLeave = async () => {
    ...
    try {
      ...
      removeTab(
        dockLayoutRef.current.state.layout.dockbox.children,
        `${data.roomStatus.roomId}번방`,
        dockLayoutRef
      );
      removeTab(
        dockLayoutRef.current.state.layout.floatbox.children,
        `${data.roomStatus.roomId}번방`,
        dockLayoutRef
      );
  ...
  const handleReady = () => {
    ...
    const updateUser = userStatus.filter(
      (user) => user.userId === localStorage.getItem('id')
    )[0];
          ...
          if (receivedMessage.leaveUserStatus) {
            return setUserStatus((prevUserStatus) =>
              prevUserStatus.filter(
                (user) => user.userId !== receivedMessage.leaveUserStatus.userId
              )
            );
          }
