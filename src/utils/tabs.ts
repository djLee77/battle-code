...
// 탭 형식에 맞게 만드는 함수
export const getTab = (id: string, component: React.JSX.Element, isClose: boolean): TabData => {
  return {
    id, // 탭의 고유한 ID
...
// 탭 제거 함수
export const removeTab = (dockLayoutRef: React.RefObject<DockLayout>, idSubstring: string) => {
  dockLayoutRef?.current?.state.layout.dockbox.children.forEach((child) => {
    if (isPanelData(child)) {
      const tabData = child.tabs?.find((tab: any) => tab.id.includes(idSubstring));
...
// 탭 추가 함수
export const addTab = (tabName: string, component: any, dockLayoutRef: React.RefObject<DockLayout>) => {
